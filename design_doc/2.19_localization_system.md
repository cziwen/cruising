...
- **职责**：
  ...
  - 如果特定语言文件不存在，自动回退到默认的 `assets/config/{filename}.json`（通常为中文，但游戏启动默认为英文）。
...
### 命名约定
- **默认文件**：`{filename}.json` (包含中文原文)
- **本地化文件**：`{filename}_{lang}.json` (如 `{filename}_en.json` 为英文)
- **默认体验**：系统启动时默认加载英文配置（通过 `_en.json` 文件），如缺失则回退至默认文件。
...
## 2.19.6 扩展建议与实现
1. **字体支持 (已实现)**：针对不同语言（如日文、韩文、中文）配置了 `ArkPixel` 像素字体族。通过在 `pubspec.yaml` 中注册多个 `.ttf` 变体，确保在全球化环境下文字显示的对齐与像素风格的一致性。
