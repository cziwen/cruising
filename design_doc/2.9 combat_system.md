...
### 战斗触发机制
- **触发时机**：在航行过程中，每经过 1 游戏小时进行一次判定
- **触发概率**：每次判定约 5% 概率触发战斗
- **优先级**：海上随机事件优先触发。若该小时已触发海上随机事件，则跳过战斗判定
- **触发条件**：
  - 必须在海上航行中（`isAtSea == true`）
  - 不在战斗中（`isInCombat == false`）
  - 战斗系统已解锁（`isCombatUnlocked == true`，通过任务动作 `combat.unlock` 解锁）
  - 当前无活跃的海上随机事件
...
4. **触发视觉更新**：
   - 玩家船只以动画方式向左移动到战斗位置（约150像素，1500ms）
   - 敌方船只从右侧滑入（1500ms）
   - 战斗中会提供受击反馈（闪白、抖动），辅助玩家感知“命中/受击”
5. **入场期间暂停结算**：入场动画期间（约1500ms）**不结算攻击/修复**，避免敌船尚未入场完毕就被秒杀导致战斗立刻结束
6. **时间继续运行**：战斗时时间不暂停，继续按照统一的游戏时钟运行
...
class EnemyShip {
  ...
  final String appearance;        // 外观图片路径
}
```
...
### 玩家船只左移
- **实现方式**：进入战斗时播放位移动画，结束后将玩家船只固定在战斗位置
- **动画时长**：1500ms
- **位置**：向左移动约150像素
- **补充**：战斗结束后播放归位动画（1500ms）

### 敌方船只滑入
- **实现方式**：场景系统使用位移动画（从屏幕右侧滑入）
- **动画时长**：1500ms
...
### 受击反馈（命中/受击提示）
- **闪白（Hit Flash）**：船只受到伤害时短暂闪白，用于强化“命中”的视觉确认
- **抖动（Shake）**：只有**受到伤害的那艘船**会短暂抖动（非全屏抖动），用于强化冲击感
- **无开火特效**：开火特效目前暂时取消（避免视觉干扰，后续可迭代更合适的表现）
- **多次战斗一致性**：每次进入新战斗会重置受击反馈相关状态，确保第二次及以上战斗中闪白/抖动依然能触发

### 敌船信息面板随动
- 敌船顶部的信息面板（耐久、攻速、修复速）会跟随敌船的上下移动
- 当敌船沉船时，信息面板也会随着沉船动画一起下沉，避免“漂浮在空中”
...
- **计算公式**：`dtGameSeconds = dtRealSeconds * currentTimeScale * timeMultiplier / 60.0`
  - `currentTimeScale` 在海上航行时为 60.0，在港口时为 1.0。
...
if (_gameState.isInCombat) {
  ...
  final dtGameSeconds = dtRealSeconds * currentTimeScale * timeMultiplier / 60.0;
...
- **时间倍数影响**：当debug panel调整时间倍数时，战斗速度也会相应变化
- **游戏时间增量**：战斗更新使用游戏时间增量（秒），计算公式：`dtGameSeconds = dtRealSeconds * currentTimeScale * timeMultiplier / 60.0`
