...
- `highlight_padding`: 高亮区域的通用内边距（可选，默认为 8）。
- `padding_top`, `padding_bottom`, `padding_left`, `padding_right`: 细粒度的高亮区域边距（可选）。如果设置，将覆盖 `highlight_padding` 的对应方向。
- `barrier_dismissible`: 任务期间打开的对话框是否允许点击外部区域关闭（可选，默认为 true）。若设置为 false，则对话框变为模态，且通常会隐藏关闭按钮。
- `text`: 展示给玩家的任务描述文本（可选）。若缺失或为 null，则不显示任务对话框。
- `complete_when`: 任务完成条件的逻辑表达式（如 `cargo.count('food') >= 10`）。
- `on_fail_hint`: 任务失败时的提示。
- `action`: 任务完成时触发的特殊动作（如 `ui.marketPanel.close`, `port.unlock('port_id')` 或 `combat.unlock`）。
- `is_mandatory`: 是否为强制任务。若为 true，系统将屏蔽高亮区域以外的所有交互。

### 1.1 可用动作 (Available Actions)
在 `action` 字段中支持以下动作（支持使用 `&&` 连接多个动作）：
- `ui.marketPanel.close`: 关闭市场/交易面板。
- `ui.shipyard.close`: 关闭船厂面板。
- `ui.hallPanel.close`: 关闭大厅面板。
- `port.unlock('port_id')`: 解锁指定的港口（如 `port.unlock('home_island')`）。
- `port.unlockAllExceptHome`: 解锁除主岛外的所有港口。
- `combat.unlock`: 解锁海上战斗（海盗随机遭遇）。
- `homeIsland.setTax(VALUE)`: 强制设置当前主岛可领取的税收金额（VALUE 为整数）。
- `homeIsland.unlockTax`: 解锁主岛税收功能。
...
- **交互控制**: 
    ...
    - 对于强制任务（`is_mandatory: true`），通过分块的 `InteractionBlocker` 屏蔽高亮区域外的点击，而高亮区域内部支持点击穿透。同时，可以通过配置 `barrier_dismissible: false` 来锁定正在进行的对话框，防止玩家通过点击外部或使用通用关闭按钮中断引导流程。
- **时间暂停**: 当任务对话框显示（即任务配置了 `text` 字段且不为 null）时，游戏时间流逝和相关逻辑（如昼夜循环、自动维修、战斗进度等）将自动暂停。这确保了玩家有充足的时间阅读引导内容，而不会在对话期间遭遇意外事件。
- **任务对话框**: 在屏幕边缘显示精美的纸张风格任务提示。若任务未配置 `text` 字段（缺失或为 null），对话框将被隐藏，适用于仅需高亮引导的场景。若配置为 `"text": ""`，则会显示一个空的对话框背景。

## 2.16.3 条件评估逻辑 (Condition Evaluation)
系统支持多种基于字符串的条件判定。支持使用 `&&`、`||` 运算符连接多个条件，并支持使用 `()` 括号嵌套。**重要：** 所有作为完成条件的 `GameState` 标志位（如 `isTradeBalanced`）必须与数据变更**同步更新**，以确保任务系统在下一帧开始前能读到正确状态。

- `tap_anywhere`: 点击屏幕任意位置完成。
- `player.gold [op] [value]`: 检查玩家金币数量（支持 `>=`, `<=`, `==`, `>`, `<`）。
...
- `ui.tradeBalanced`: 检查交易是否平衡（商人接受）。
- `ui.tradeConfirmed`: 检查交易是否已点击确认并成功执行。
- `trade.pending('goods_id') [op] [value]`: 检查待交易清单中的物品数量（换入为正，换出为负）。
- `ui.shipyard.opened`: 检查船厂对话框是否打开。
- `ui.hallPanel.opened`: 检查主岛大厅对话框是否打开。
- `ui.homeIslandUpgraded`: 检查主岛是否执行了至少一次升级。
- `cargo.count('goods_id') >= value`: 检查船只库存。
- `navigation.destination == 'port_id'`: 检查航行目的地。
- `navigation.map_selected == 'port_id'`: 检查地图对话框中是否选中了特定港口。
- `navigation.arrived`: 检查是否到达目的地（不在海上且场景切换已完成）。
- `navigation.isAtSea`: 检查船只是否正在海上航行。
- `navigation.current_port == 'port_id'`: 检查当前所在港口。
- `navigation.current_port != 'port_id'`: 检查当前不在特定港口。
- `navigation.visited_ports_count [op] [value]`: 检查已访问过的港口总数。
- `ship.cargo_level >= value`: 检查船只载货量等级。
- `ship.level >= value`: 检查船只整体等级（所有部件最低级）。
- `ship.upgrade_count >= value`: 检查船只总升级次数。
- `homeIsland.opened`: 检查当前是否在主岛。
- `!homeIsland.opened`: 检查当前是否不在主岛。
...
### 1. 触发器 (`trigger`)
控制任务何时被激活。支持使用 `&&`、`||` 和 `()` 组合多个条件。
- `on_new_game_start`: 新游戏开始且没有进行中的任务时触发。
- `after(quest_id)`: 在指定 ID 的任务完成后立即触发。
- **状态触发**: 触发器可以直接使用 **2.16.3** 中列出的所有条件表达式（如 `navigation.arrived`, `player.gold >= 3000` 等）。
- **复合触发器**: 示例：`after(T05_sailing) && navigation.arrived`。

### 2. 完成条件 (`complete_when`)
定义玩家需要执行什么操作来完成任务。支持使用 `&&`、`||` 和 `()` 连接多个子条件。
...
- `ui.tradeConfirmed`: 当交易确认并成功执行时完成。
- `ui.shipyard.opened`: 当船厂对话框打开时完成。
- `ui.hallPanel.opened`: 当主岛大厅对话框打开时完成。
- `ui.homeIslandUpgraded`: 当主岛执行了至少一次升级时完成。
- `cargo.count('goods_id') [op] [value]`: 检查货物数量（支持的 ID 见 `goods.json`）。
- `navigation.destination == 'port_id'`: 当玩家在目的地列表中选择了特定港口时完成。
- `navigation.map_selected == 'port_id'`: 当玩家在地图上点击了特定港口时完成。
...
- `ui.mapPort('port_id')`: 地图对话框中的特定港口图标。
- `ui.mapDepartButton`: 地图对话框中的“出发”按钮。
- `ui.portDepartButton('port_id')`: 目的地列表中的特定港口的“出发”按钮。

### 4. 任务文本 (`text`)
展示给玩家的剧情或指引文本。
- **缺失/Null**: 隐藏整个任务对话框。适用于仅需要 UI 高亮指引，不希望文字遮挡视线的步骤。
- **空字符串 `""`**: 显示一个空的纸张对话框背景。
- **普通文本**: 正常显示文字描述。
